...
cp -r scripts/arkit dist/scripts/
cp -r scripts/vtserver dist/scripts/
...
touch dist/vtserver.bat
echo 'copy /y bin\cxluax.exe  bin\vtserver.exe&start bin/vtserver.exe  --cwd=%cd%/ --script_path=scripts/vtserver/' > dist/vtserver.bat
