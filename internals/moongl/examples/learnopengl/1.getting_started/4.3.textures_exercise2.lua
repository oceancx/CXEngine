...
local prog, vsh, fsh = gl.make_program({vertex="shaders/4.3.texture.vert",
                                         fragment="shaders/4.3.texture.frag"})
...
--  note that we set the container wrapping mode to 'clamp to edge':
...
-- we want to repeat the awesomeface pattern so we keet the wrapping mode to 'repeat':
