...
local prog, vsh, fsh = gl.make_program({vertex="shaders/5.2.transform.vert",
                                         fragment="shaders/5.2.transform.frag"})
...
local transform_loc = gl.get_uniform_location(prog, "transform")
...
while not glfw.window_should_close(window) do
   ...
   local transform = glmath.translate(0.5, -0.5, 0.0)*glmath.rotate(t, 0.0, 0.0, 1.0)
   gl.uniform_matrix4f(transform_loc, true, transform)
   gl.draw_elements('triangles', 6, 'uint', 0)
   -- draw the second container, with a different transform matrix:
   local s = math.sin(t) -- scale factor
   transform = glmath.translate(-0.5, 0.5, 0.0)*glmath.scale(s)
   gl.uniform_matrix4f(transform_loc, true, transform)
   gl.draw_elements('triangles', 6, 'uint', 0)
