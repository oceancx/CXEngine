...
local mixValue = 0.2 -- how much we're seeing of either texture
local mixIncr = 0.001 -- how much to increase/decrease mixValue upon key up/down input
-- May need tuning, since it might be too slow or too fast depending on the frame rate.
...
local mixValue_loc = gl.get_uniform_location(prog, "mixValue")
...
while not glfw.window_should_close(window) do
   ...
   if glfw.get_key(window, 'up') == 'press' then
      mixValue = mixValue + mixIncr
      if mixValue > 1.0 then mixValue = 1.0 end
   end
   if glfw.get_key(window, 'down') == 'press' then
      mixValue = mixValue - mixIncr
      if mixValue < 0.0 then mixValue = 0.0 end
   end
   ...
   gl.uniformf(mixValue_loc, mixValue)
