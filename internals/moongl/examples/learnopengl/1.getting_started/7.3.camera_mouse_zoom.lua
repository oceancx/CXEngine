...
local yaw = -90.0 -- yaw=0 would result in pointing to the right (+x axis)
local pitch = 0.0
...
local prog, vsh, fsh = gl.make_program({vertex="shaders/7.3.camera.vert",
                                         fragment="shaders/7.3.camera.frag"})
...
glfw.set_cursor_pos_callback(window, function(window, xpos, ypos)
   ...
   local yaw, pitch = rad(yaw), rad(pitch)
   cam_front = vec3(cos(yaw)*cos(pitch), sin(pitch), sin(yaw)*cos(pitch)):normalize()
end)
...
while not glfw.window_should_close(window) do
   ...
   local view = look_at(cam_pos, cam_pos+cam_front, cam_up)
   gl.uniform_matrix4f(view_loc, true, view)
   -- projection transform (in this example it may change at every frame):
   local projection = perspective(rad(fov), SCR_WIDTH/SCR_HEIGHT, 0.1, 100.0)
