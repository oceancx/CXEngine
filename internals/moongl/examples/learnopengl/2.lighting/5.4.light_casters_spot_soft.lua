...
local prog1, vsh, fsh = gl.make_program({vertex="shaders/5.4.light_casters.vert",
                                         fragment="shaders/5.4.light_casters.frag"})
gl.delete_shaders(vsh, fsh)
-- lamp shader program:
local prog2, vsh, fsh = gl.make_program({vertex="shaders/5.4.lamp.vert",
                                         fragment="shaders/5.4.lamp.frag"})
gl.delete_shaders(vsh, fsh)
...
loc1["light.position"] = gl.get_uniform_location(prog1, "light.position")
...
loc1["light.constant"] = gl.get_uniform_location(prog1, "light.constant")
loc1["light.linear"] = gl.get_uniform_location(prog1, "light.linear")
loc1["light.quadratic"] = gl.get_uniform_location(prog1, "light.quadratic")
loc1["light.cutOff"] = gl.get_uniform_location(prog1, "light.cutOff")
loc1["light.outerCutOff"] = gl.get_uniform_location(prog1, "light.outerCutOff")
...
while not glfw.window_should_close(window) do
   ...
   gl.uniformf(loc1["light.position"], camera.position)
   gl.uniformf(loc1["light.direction"], camera.front)
   gl.uniformf(loc1["light.cutOff"], cos(rad(12.5)))
   gl.uniformf(loc1["light.outerCutOff"], cos(rad(17.5)))
   ...
   -- with each lighting method and environment. Each environment and lighting type requires
   -- some tweaking to get the best out of your environment.
   gl.uniformf(loc1["light.diffuse"],  0.8, 0.8, 0.8)
   gl.uniformf(loc1["light.ambient"],  0.1, 0.1, 0.1)
   gl.uniformf(loc1["light.specular"], 1.0, 1.0, 1.0)
   gl.uniformf(loc1["light.constant"], 1.0)
   gl.uniformf(loc1["light.linear"], 0.09)
   gl.uniformf(loc1["light.quadratic"], 0.032)
   -- material properties
