...
local prog1, vsh, fsh = gl.make_program({vertex="shaders/3.1.materials.vert",
                                         fragment="shaders/3.1.materials.frag"})
gl.delete_shaders(vsh, fsh)
-- lamp shader program:
local prog2, vsh, fsh = gl.make_program({vertex="shaders/3.1.lamp.vert",
                                         fragment="shaders/3.1.lamp.frag"})
gl.delete_shaders(vsh, fsh)
...
loc1["material.ambient"] = gl.get_uniform_location(prog1, "material.ambient")
...
while not glfw.window_should_close(window) do
   ...
   gl.uniformf(loc1["light.position"], light_pos)
   gl.uniformf(loc1.viewPos, camera.position)
   -- light properties
   local light_color = vec3(sin(t*2.0),sin(t*0.7), sin(t*1.3))
   gl.uniformf(loc1["light.diffuse"],  0.5*light_color) -- decrease the influence
   gl.uniformf(loc1["light.ambient"],  0.1*light_color) -- still lower influence
   gl.uniformf(loc1["light.specular"], 1.0, 1.0, 1.0)
   -- material properties
   gl.uniformf(loc1["material.ambient"], 1.0, 0.5, 0.31)
   gl.uniformf(loc1["material.diffuse"], 1.0, 0.5, 0.31)
   gl.uniformf(loc1["material.specular"], 0.5, 0.5, 0.5) -- no effect on this object's material
