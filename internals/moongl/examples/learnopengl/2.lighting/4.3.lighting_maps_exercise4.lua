...
-- lamp shader program:
local prog2, vsh, fsh = gl.make_program({vertex="shaders/4.3.lamp.vert",
                                         fragment="shaders/4.3.lamp.frag"})
gl.delete_shaders(vsh, fsh)
...
local emission_map = load_texture("../resources/textures/matrix.jpg")
...
loc1["material.emission"] = gl.get_uniform_location(prog1, "material.emission")
...
while not glfw.window_should_close(window) do
   ...
   gl.uniformi(loc1["material.emission"], 2) -- texture unit
   gl.uniformf(loc1["material.shininess"], 64.0)
   ...
   -- bind diffuse, specular, and emission maps
   gl.active_texture(0)
   gl.bind_texture('2d', diffuse_map)
   gl.active_texture(1)
   gl.bind_texture('2d', specular_map)
   gl.active_texture(2)
   gl.bind_texture('2d', emission_map)
   -- draw
