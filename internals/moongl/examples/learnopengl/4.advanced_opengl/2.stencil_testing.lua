...
local prog, vsh, fsh = gl.make_program({vertex="shaders/2.stencil_testing.vert",
                                         fragment="shaders/2.stencil_testing.frag"})
gl.delete_shaders(vsh, fsh)
local prog1, vsh, fsh = gl.make_program({vertex="shaders/2.stencil_testing.vert",
                                         fragment="shaders/2.stencil_single_color.frag"})
gl.delete_shaders(vsh, fsh)
...
while not glfw.window_should_close(window) do
   ...
   gl.clear('color', 'depth', 'stencil')  -- don't forget to clear the stencil buffer!

   local view = camera:view()
   local projection = perspective(rad(camera.zoom), SCR_WIDTH/SCR_HEIGHT, 0.1, 100.0)

   -- draw floor as normal, but don't write the floor to the stencil buffer, we only care
   -- about the containers. We set its mask to 0x00 to not write to the stencil buffer.
   gl.use_program(prog)
   ...
   gl.bind_vertex_array(cube_vao)
   gl.uniform_matrix4f(loc1.model, true, translate(-1.0, 0.0, -1.0)*scale(1.1))
   gl.draw_arrays('triangles', 0, 36)
   gl.uniform_matrix4f(loc1.model, true, translate(2.0, 0.0, 0.0)*scale(1.1))
   gl.draw_arrays('triangles', 0, 36)
