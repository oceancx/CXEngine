...
local prog, vsh, fsh = gl.make_program({vertex="shaders/3.2.blending.vert",
                                         fragment="shaders/3.2.blending.frag"})
...
local function sort_windows() -- sorts windows (in place) from furthest to nearest to the camera
   table.sort(windows, function(a, b) return (camera.position-a):norm2()>(camera.position-b):norm2() end)
end

-- load textures
local cube_texture = new_texture("../resources/textures/marble.jpg")
local floor_texture = new_texture("../resources/textures/metal.png")
local transparent_texture = new_texture("../resources/textures/window.png")
...
while not glfw.window_should_close(window) do
   ...
   gl.bind_texture('2d', transparent_texture)
   for _, pos in ipairs(windows) do
