...
local prog, vsh, fsh = gl.make_program({vertex="shaders/5.2.framebuffers.vert",
                                         fragment="shaders/5.2.framebuffers.frag"})
gl.delete_shaders(vsh, fsh)
local prog1, vsh, fsh = gl.make_program({vertex="shaders/5.2.framebuffers_screen.vert",
                                         fragment="shaders/5.2.framebuffers_screen.frag"})
gl.delete_shaders(vsh, fsh)
...
while not glfw.window_should_close(window) do
   ...
   -- bind to framebuffer and draw to color texture as we normally  would, but with the
   -- view camera reversed.
   ...
   gl.use_program(prog)
   -- rotate the camera's yaw and pitch 180 degrees around, then update the camera vectors
