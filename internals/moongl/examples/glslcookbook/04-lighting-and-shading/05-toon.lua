...
local TITLE = "Chapter 4 - Toon shading"
local W, H = 800, 600
...
local vertex_shader = "shaders/toon.vert"
local fragment_shader = "shaders/toon.frag"
...
local view = glmath.look_at(vec3(4,4,6.5), vec3(0,.75,0), vec3(0,1,0))
...
local function set_material(kd, ka)
...
while not glfw.window_should_close(window) do
   ...
   set_material(vec3(0.9, 0.5, 0.3), vec3(0.9, 0.5, 0.3)*.3)
   ...
   set_material(vec3(0.7, 0.7, 0.7), vec3(0.2, 0.2, 0.2)*.3)
   local model = mat4()
   set_matrices(model)
   plane:render()
