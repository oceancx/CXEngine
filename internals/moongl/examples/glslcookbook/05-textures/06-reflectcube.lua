...
local TITLE = "Chapter 5 - Reflection with a cube map"
local W, H = 800, 600
...
local prog, vsh, fsh = gl.make_program({
   vertex = "shaders/cubemap_reflect.vert", 
   fragment = "shaders/cubemap_reflect.frag"
})
...
while not glfw.window_should_close(window) do
   ...
   gl.uniformf(loc["MaterialColor"], .5, .5, .5, 1.0)
   gl.uniformf(loc["ReflectFactor"], .85)
