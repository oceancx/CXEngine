...
subroutine(RenderPassType) void pass1()
    ...
    if(nodeIdx < MaxNodes)
        {
        // Our fragment will be the new head of the linked list, so replace the value at
        // gl_FragCoord.xy with our new node's index.  We use imageAtomicExchange to make
        // sure that this is an atomic operation.  The return value is the old head of
        // the list (the previous value), which will become the next element in the list
        // once our node is inserted.
        uint prevHead = imageAtomicExchange(headPointers, ivec2(gl_FragCoord.xy), nodeIdx);
        // Here we set the color and depth of this new node to the color and depth of the
        // fragment.  The next pointer, points to the previous head of the list.
