...
subroutine(shadeModelType) vec3 phongModel(vec3 position, vec3 n)
    {
...
subroutine(shadeModelType) vec3 diffuseOnly(vec3 position, vec3 n)
    {
    vec3 s = normalize(Light.Position.xyz - position);
...
void main()
    ...
    // Evaluate the shading equation. 
    // This will call one of the functions: diffuseOnly or phongModel.
