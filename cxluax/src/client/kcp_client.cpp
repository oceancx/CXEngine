...
void kcp_client::do_send_connect_packet(uint64_t cur_clock)
{
    ...
    const int send_ret = send(udp_socket_, connect_msg.c_str(), connect_msg.size(), 0);
    if (send_ret < 0)
    {
...
void kcp_client::try_recv_connect_back_packet(void)
{
    ...
    const int ret_recv = recv(udp_socket_, recv_buf, sizeof(recv_buf), 0); // recv timeout is 2 milliseconds. - SO_RCVTIMEO
    if (ret_recv < 0)
    {
        int err = errno;
        if (err == EAGAIN)
...
int kcp_client::init_udp_connect(void)
{
    ...
    {
        /*int flags = ioctlsocket(udp_socket_, FIOASYNC, 0);
        if (flags == -1)
        {
        ...
        int ret = ioctlsocket(udp_socket_, FIONBIO, 0);
        if (ret == -1)
        {
...
void kcp_client::do_recv_udp_packet_in_loop(void)
{
    ...
    const int ret_recv = recv(udp_socket_, recv_buf, sizeof(recv_buf), 0);
    if (ret_recv < 0)
    {
        int err = errno;
        if (err == EAGAIN)
...
void kcp_client::send_udp_package(const char *buf, int len)
{
    ...
    const int send_ret = send(udp_socket_, buf, len, 0);
    if (send_ret < 0)
    {
