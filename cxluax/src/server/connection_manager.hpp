...
{
    NONCOPYABLE(connection_manager);
public:

    typedef std::shared_ptr<connection_manager> shared_ptr;

    connection_manager(asio::io_service& io_service, const std::string& address, int udp_port);
...
private:
    ...
    /// The UDP
    void handle_udp_receive_from(const asio::error_code& error, size_t bytes_recvd);
...
private:
    ...
    asio::high_resolution_timer kcp_timer_;
    uint32_t cur_clock_;
